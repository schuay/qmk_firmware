...
#endif
    // Tracks the last *attempted* send. The driver send is void-returning
    // and best-effort (queues to a USB or BLE endpoint buffer), so a
    // successful return doesn't guarantee transmission. Good enough as a
    // shadow for callers that need to stamp held-button state onto
    // out-of-band mouse reports.
